        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="flex justify-center items-center mb-12 space-x-6">
            <button 
              ...
              className="w-12 h-12 rounded-full bg-white shadow-md hover:shadow-lg transition-all duration-300 border border-gray-200 flex items-center justify-center"
            ...
            <div className="flex items-center space-x-2">
              {Array.from({ length: Math.ceil(testimonials.length / testimonialsPerPage) }, (_, i) => (
                <button
                  ...
                  style={{
                    backgroundColor: Math.floor(currentIndex / testimonialsPerPage) === i ? '#2ca8e0' : undefined
                  }}
                />
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {currentTestimonials.map((testimonial) => (
              <div 
                key={testimonial.id} 
                className="bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transition-shadow duration-300 border-l-4"
                style={{ borderLeftColor: '#2ca8e0' }}
              >
                {/* Quote and Stars */}
                <div className="flex items-center justify-between mb-4">
                  <span className="text-2xl text-blue-400" style={{ color: '#2ca8e0' }}>"</span>
                ...
                <div className="flex items-center mb-4">
                  <div 
                    className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-bold mr-4 ${getAvatarColor(testimonial.id)}`}
                  >
                    {testimonial.initial}
                  </div>
                  <div>
